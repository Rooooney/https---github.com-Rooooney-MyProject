...
- (void)viewDidLoad {
    ...
    [self.view addGestureRecognizer:({//添加轻击出现菜单栏的手势
...
-(void)showToolMenu:(UITapGestureRecognizer*)sender
{
    ...
    if (point.x > ViewSize(self.view).width/4 && point.x < self.view.frame.size.width-ViewSize(self.view).width/4) {//只有当点击区域在屏幕的中间区域时
...
{
    if([_model.content isEqualToString:@"pdf"]){
...
-(void)menuViewMark:(LSYTopMenuView *)topMenu
{
    ...
    if (state) {
        ...
        model.date = [LSYReadUtilites getCurrentDate];
        model.recordModel = [_model.record copy];
...
- (nullable UIViewController *)pageViewController:(UIPageViewController *)pageViewController viewControllerAfterViewController:(UIViewController *)viewController
{
    ...
    _pageChange = _page;
    _chapterChange = _chapter;
    if (_pageChange == _model.chapters.lastObject.pageCount-1 && _chapterChange == _model.chapters.count-1) {//最后一页
        [LSYReadUtilites showAlertTitle:nil content:@"最后一页啦"];
        return nil;
    }
...
-(void)viewDidLayoutSubviews
{
    ...
    _catalogView.frame = CGRectMake(-ViewSize(self.view).width, 0, 2*ViewSize(self.view).width, ViewSize(self.view).height);
    _catalogVC.view.frame = CGRectMake(0, 0, ViewSize(self.view).width*2/3, ViewSize(self.view).height);
    [_catalogVC reload];
}

...
- (void)configBookCellModel:(BookModel *)model
{
    ...
    if(model.bookType == BookTypePDF){
        ...
        if(coverimg==nil){//如果没有封面，就解压文件
            ...
            //[LSYReadModel getLocalModelWithURL:[NSURL URLWithString:[model.filePath stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]];
            [LSYReadModel getLocalModelWithBookModel:model];
